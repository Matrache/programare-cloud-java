...
public class UserDataSource {
    ...
    public Set<String> getAllUserJobsSorted(){
        return users.stream()
                .sorted(Comparator.comparing(User::getJob))
                .map(User::getJob)
    ...
    public void changeAllStudentsJobsAndAges(){
        users.stream()
                ...
                .forEach(user -> {user.setJob("graduate"); user.setAge(user.getAge() + 5);});
    }
    ...
    public Predicate<User> getPredicateForFilteringByName(String name){
        return user -> user.getLastName().equals(name) || user.getFirstName().equals(name);
    }
